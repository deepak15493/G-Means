...
//define cluster as a matrix and store number of data points, Anderson Darling test score and the centroid
...
void performKmean(int nrows, int ncols, double** data, int** mask){
	...
	do{
		...
		// to count the cluster assignment
		...
		// dot product of each cluster with vector v
		double x1[nrows];
		for(int i=0;i<nrows;i++)
		{
	...
	for(int k=0; k<clusterCounter; k++){
		printf(" Data points in cluster %d: %d", k, cluster_array[k].nrows);
	}
